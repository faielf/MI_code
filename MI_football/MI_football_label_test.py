...
for i in range(cnt_num):
    file="F:/MNE-zwx-test/zwx_test{}.cnt".format(i+1)
    raw_cnt = mne.io.read_raw_cnt(file,
            eog=['HEO', 'VEO'], emg=['EMG'], ecg=['EKG'],
            preload=True, verbose=False)
    ref_channels=['M1', 'M2']
    srate=1000
    mne.set_eeg_reference(raw_cnt,ref_channels)
    events, event_id = mne.events_from_annotations(raw_cnt)
    channels=copy.deepcopy(raw_cnt.ch_names)
    channels.remove('HEO')
    channels.remove('VEO')
    channels.remove('EMG')
    channels.remove('EKG')
    ...
    epochs = mne.Epochs(raw_cnt, events, event_id, tmin, tmax, proj=True, picks=picks,
    ...
    for limb_n in range(len(limb_channels)):
        limb_index[limb_n]=channels.index(limb_channels[limb_n])
    limb_index=limb_index.astype(int)
    if i==0:
        data=epochs.get_data()[:,limb_index,:]
        label=(events[:,-1]-1).astype(int)
    if i>=1:
        data=np.concatenate((data,epochs.get_data()[:,limb_index,:]),axis=0)
        label=np.concatenate((label,(events[:,-1]-1).astype(int)),axis=0)
 
    
Online_filter=OnlineBlockFilter(srate=srate,filters=[[4,30]])
Online_filter.fit()
f_X=np.squeeze(Online_filter.transform(data))   
X_mat=cov_matrix(f_X)
TSSVMC_X=X_mat.copy()
y=label.copy()
TSSVMC=RGC.TSclassifier()
TSSVMC_FPR_result=[]
TSSVMC_ACC_result=[]
...
TSSVMC_ACC_mean=np.mean(TSSVMC_ACC_result)
TSSVMC_FPR_mean=np.mean(TSSVMC_FPR_result)
...
for k in range(1,10):
    ...
    TSSVMC=RGC.TSclassifier(clf=svm.SVC(C=1.0))
    TSSVMC_FPR_result=[]
    TSSVMC_ACC_result=[]
...
fin_FPR_result=np.around(np.array((mean_TSSVMC_all_FPR_result,mean_TSSVMC1_all_FPR_result,mean_TSSVMC2_all_FPR_result,
...
fin_ACC_result=np.around(np.array((mean_TSSVMC_all_ACC_result,mean_TSSVMC1_all_ACC_result,mean_TSSVMC2_all_ACC_result,
...
plt.ylabel('False positive rate/%')
plt.grid(axis='x')
plt.bar(sub_num-2*width_val,mean_TSSVMC_all_FPR_result,width=width_val,label='C=0.5')
...
plt.ylabel('Accuracy/%')
plt.grid(axis='x')
plt.bar(sub_num-2*width_val,mean_TSSVMC_all_ACC_result,width=width_val,label='C=0.5')
